...
Based on the test output is there sufficient evidence to conclude that there is at least one difference between the group means? Enter Yes or No.
```{r text_box, echo = FALSE}
question_text(
  "Type your conclusion below:",
  answer("no", message = "Recall that if $p_{obs}$, the number listed under Pr(>F), is less than the alpha value
  ...
  answer("No", message = "Recall that if $p_{obs}$, the number listed under Pr(>F), is less than the alpha value
         there is sufficient evidence to conclude that there is at least one difference between the group means. 
         However, $p_{obs}$ is 0.469 in this case and greater than the alpha value."),
  answer("yes", correct = TRUE, message = "The p-value corresponding to the F-stat is 0.469. This is greater than the
